...
class Stat extends Component {
  ...
  drawVisitChart() {
    const defaultRange = { hour: 24, day: 7 };
    const visitChart = echarts.init(this.charts.visit);
    const data = this.data.visit[this.state.visit.interval];
  ...
  componentDidMount() {
    ...
    const accessChart = echarts.init(this.charts.access);
    ...
    const amountChart = echarts.init(this.charts.amount);
    const amountMin = 1;
    const amountMax = 100;
    StatUtils.getAmount().then((data) => {
      ...
      const tmp = {};
      let j = 0;
      for (let i = amountMax; i >= amountMin; i--) {
        tmp[i] = tmp[i + 1] || 0;
    ...
    const labelsChart = echarts.init(this.charts.labels);
    const options = Config.options.map(item => item.key);
    const values = [
    ...
    StatUtils.getLabels().then((data) => {
      labelsChart.setOption({
        tooltip: {
          ...
          formatter(params) {
            return [params[0].axisValueLabel].concat(params.filter(param => param.data).map(param => `${param.marker}<span>${param.seriesName}:${param.data}</span>`)).join('<br>');
          },
        },
        grid: {
          left: '3%',
